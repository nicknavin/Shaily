 ...
 * Use the {@link HomeStoryFragment#newInstance} factory method to
...
String category_cd="";
    ...
    public static HomeStoryFragment newInstance(String param1) {
        HomeStoryFragment fragment = new HomeStoryFragment();
...
public void getFilter(String key)
{
    ...
    public void getUserByCategory()
    {
        if (Utility.isConnectingToInternet(getContext()))
        {
            ...
            ReqGetUser reqGetUser=new ReqGetUser();
            reqGetUser.setCategory_id(category_cd);

            ApiInterface apiInterface = ApiClientExplore.getClient().create(ApiInterface.class);
            Call<ConsultUserRoot> call = apiInterface.reqGetUsers(accessToken, reqGetUser);
            call.enqueue(new Callback<ConsultUserRoot>() {
                ...
                public void onResponse(Call<ConsultUserRoot> call, Response<ConsultUserRoot> response)
                {
                    if(response.body()!=null)
                    {
                        ...
                        {
                            ArrayList<ConsultUser> consultUserArrayList=response.body().getConsultUsersList();
...
//                            }
                            demoAdapter=new DemoAdapter(getContext(),userId, consultUserArrayList, new ApiItemCallback() {
                                ...
                                {
                                    startActivity(new Intent(getContext(), ExpertProfilePageActivity.class));
                                }
                            });
