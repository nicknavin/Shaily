...
public class ShowGroupStoryActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(!userStory.getUserId().getId().equals(userId))
        {
            sendStoryView();
        }
    ...
    private void setupViewModel() {
        viewModel = new ViewModelProvider(this, new ViewModelFactory( userId,accessToken)).get(ShowGroupStoryViewModel.class);
      // binding.setGroup(viewModel);
      //  binding.setLifecycleOwner(this);

        RequestReadCount readCount=new RequestReadCount();
        readCount.setStory_id(userStory.get_id());
        readCount.setStory_provider(userStory.getStory_provider());
        readCount.setSubscription_id(userStory.getSubscriptionId().get_id());
        readCount.setUser_id(userId);
        viewModel.readViewCount(readCount);
    }
...
else
{
    laySubscription.setVisibility(View.VISIBLE);
        ...
        header.setText(userStory.getUserId().getUserName());
        String userUrl=userStory.getUserId().getImageUrl();
        if(userUrl!=null&&!userUrl.isEmpty()&&!userUrl.equals("null"))
        {
            userUrl=Urls.BASE_IMAGES_URL+userStory.getUserId().getImageUrl();
...
//                    apiCallback.getObject(data, position, view);
    ...
    private void sendStoryView()
    {
        if(isInternetConnected())
        {
            ...
            storyId.setStory_id(userStory.get_id());
...
//                        startActivity(intent);
        ...
        if(userId.equals(userStory.getUserId().getId()))
        {
